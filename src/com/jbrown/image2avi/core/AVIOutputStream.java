...
import com.jbrown.image2avi.chunk.DataChunk;
import com.jbrown.image2avi.chunk.FixedSizeDataChunk;
import com.jbrown.image2avi.utils.KeysI;

public class AVIOutputStream {
    private ImageOutputStream _out;
    private VideoFormat videoFormat;

    private float _jpgEncodedVideoFrameQuality = 0.9f;
    private Date _movieCreationTime;
    ...
    public AVIOutputStream(File file, VideoFormat format) throws IOException {
        ...
        _out = new FileImageOutputStream(file);
    ...
    public void setVideoCompressionQuality(float newValue) {
        this._jpgEncodedVideoFrameQuality = newValue;
    }
    ...
    public float getVideoCompressionQuality() {
        return _jpgEncodedVideoFrameQuality;
    }
    ...
    public void writeFrame(BufferedImage image) throws IOException {
        ...
        if (_imageWidth == -1) {
            ...
            if (_imageWidth != image.getWidth() || _imageHeight != image.getHeight()) {
        ...
        DataChunk videoFrameChunk = new DataChunk(_out, (videoFormat==VideoFormat.RAW)?"00db":"00dc");
        moviChunk.add(videoFrameChunk);
        long offset = _out.getStreamPosition();
        switch (videoFormat) {
            case RAW: {
                WritableRaster raster = image.getRaster();
                int[] raw = new int[_imageWidth * 3]; // holds a scanline of raw image data with 3 channels of 32 bit data
                byte[] bytes = new byte[_imageWidth * 3]; // holds a scanline of raw image data with 3 channels of 8 bit data
                for (int y = _imageHeight-1;y>=0; y--) { // Upside down
                    raster.getPixels(0, y, _imageWidth, 1, raw);
                    for (int x = 0,n= _imageWidth*3;x<n; x+=3) {
            ...
            case JPG: {
                ...
                iwParam.setCompressionQuality(_jpgEncodedVideoFrameQuality);
        ...
        long length = _out.getStreamPosition() - offset;
        videoFrameChunk.finish();

        videoFrames.add(new Sample(_frameRate, offset, length));
        if (_out.getStreamPosition() > 1L << 32) {
    ...
    public void writeFrame(InputStream in) throws IOException {
        ...
        DataChunk videoFrameChunk = new DataChunk(_out, "00dc");
        moviChunk.add(videoFrameChunk);
    ...
    private void writeProlog() throws IOException {
        ...
        aviChunk = new CompositeChunk(_out, "RIFF", "AVI ");
        CompositeChunk hdrlChunk = new CompositeChunk(_out, "LIST", "hdrl");

        // Write empty AVI Main Header Chunk - we fill the data in later
        aviChunk.add(hdrlChunk);
        avihChunk = new FixedSizeDataChunk(_out, "avih", 56);
        avihChunk.seekToEndOfChunk();
        hdrlChunk.add(avihChunk);

        CompositeChunk strlChunk = new CompositeChunk(_out, "LIST", "strl");
        hdrlChunk.add(strlChunk);

        // Write empty AVI Stream Header Chunk - we fill the data in later
        strhChunk = new FixedSizeDataChunk(_out, "strh", 56);
        strhChunk.seekToEndOfChunk();
        strlChunk.add(strhChunk);
        strfChunk = new FixedSizeDataChunk(_out, "strf", 40);
        strfChunk.seekToEndOfChunk();
        strlChunk.add(strfChunk);

        moviChunk = new CompositeChunk(_out, "LIST", "movi");
    ...
    private void writeEpilog() throws IOException {
         ...
         */
        DataChunk idx1Chunk = new DataChunk(_out, KeysI.DW_CHUNK_NAME);
        aviChunk.add(idx1Chunk);
        d = idx1Chunk.getOutputStream();

        for (Sample f : videoFrames) {
            d.writeType(KeysI.DW_CHUNK_ID); // dwChunkId
            ...
            d.writeUInt(KeysI.DW_CHUNK_FLAG); // dwFlags
        ...
        d.writeUInt((1000000L * (long) _timeScale) / (long) _frameRate); // dwMicroSecPerFrame
        ...
        d.writeUInt(_imageWidth); // dwWidth
        // Specifies the width of the AVI file in pixels.

        d.writeUInt(_imageHeight); // dwHeight
        ...
        switch (videoFormat) {
            case RAW:
                d.writeType(KeysI.FCC_HANDLER_WRITE_TYPE_DIB);
                break;
            case JPG:
                d.writeType(KeysI.FCC_HANDLER_WRITE_TYPE_MJPG);
                break;
            case PNG:
            default:
                d.writeType(KeysI.FCC_HANDLER_WRITE_TYPE_PNG);
                break;
        }
        ...
        d.writeUInt(_timeScale); // dwScale
        ...
        d.writeUShort(_imageWidth); // rcFrame.right
        d.writeUShort(_imageHeight); // rcFrame.bottom
        ...
        d.writeInt(_imageWidth); // biWidth
        // Specifies the width of the bitmap, in pixels.

        d.writeInt(_imageHeight); // biHeight
        ...
        switch (videoFormat) {
            case RAW:
                ...
                break;
            case JPG:
                d.writeType(KeysI.FCC_HANDLER_WRITE_TYPE_MJPG); // biCompression - MJPG for Motion JPEG
                break;
            case PNG:
            default:
                d.writeType(KeysI.FCC_HANDLER_WRITE_TYPE_PNG); // biCompression - png for PNG
                break;
        }
        ...
        d.writeInt(_imageWidth * _imageHeight * 3); // biSizeImage
